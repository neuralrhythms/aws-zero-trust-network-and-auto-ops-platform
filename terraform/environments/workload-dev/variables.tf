# =============================================================================
# Input variables — workload-dev environment
#
...
variable "tf_state_bucket" {
  description = "Name of the S3 bucket used to store Terraform state for this workload-dev environment. Must already exist and have versioning enabled."
  type        = string
}
...
variable "vpc_cidr" {
  description = "Primary IPv4 CIDR block for the workload-dev VPC. Must be an RFC 1918 range and must not overlap with the network-hub or other spoke VPCs."
  type        = string
}
...
variable "cluster_name" {
  description = "Name of the EKS cluster for this workload-dev environment. Also used as the vpc_name tag for derived network resources."
  type        = string
}
...
variable "ecr_repository_names" {
  description = "List of ECR repository names to create in this workload-dev account. Each repository gets immutable tags, scan-on-push, and a 30-day untagged image expiry policy."
  type        = list(string)
}
