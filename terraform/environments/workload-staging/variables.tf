# =============================================================================
# Input variables — workload-staging environment
#
...
variable "aws_region" {
  description = "AWS region in which all resources for this workload-staging account are deployed (e.g. 'eu-west-1')."
  type        = string
}

variable "tf_state_bucket" {
  description = "Name of the S3 bucket used to store Terraform state for this workload-staging environment. Must already exist and have versioning enabled."
  type        = string
}
...
variable "vpc_cidr" {
  description = "Primary IPv4 CIDR block for the workload-staging VPC. Must be an RFC 1918 range and must not overlap with the network-hub or other spoke VPCs."
  type        = string
}
...
variable "cluster_name" {
  description = "Name of the EKS cluster for this workload-staging environment. Also used as the vpc_name tag for derived network resources."
  type        = string
}
...
variable "linux_instance_types" {
  description = "EC2 instance types for the Linux (AL2) managed node group. Sourced from terraform.tfvars — never hardcoded in main.tf or this file (REQ-5.5). Staging uses m6i family for production-grade instance uplift."
  type        = list(string)
...
variable "windows_instance_types" {
  description = "EC2 instance types for the Windows (Server 2019 Core) managed node group. Sourced from terraform.tfvars — never hardcoded in main.tf or this file (REQ-5.5). Staging uses m6i family for production-grade instance uplift."
  type        = list(string)
...
variable "ecr_repository_names" {
  description = "List of ECR repository names to create in this workload-staging account. Each repository gets immutable tags, scan-on-push, and a 30-day untagged image expiry policy."
  type        = list(string)
}
