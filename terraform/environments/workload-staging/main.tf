################################################################################
# Environment: workload-staging
# Account role: Staging EKS cluster and workloads (Workloads BU OU)
#
# Module composition:
...
#
# KEY DIFFERENCE from dev/test: production-grade m6i instance family is used
# for both Linux and Windows node groups (REQ-5.4). This environment validates
# workloads against production-grade compute before promotion to workload-prod.
#
...
# ------------------------------------------------------------------------------
# EKS cluster — heterogeneous managed node groups (staging: m6i family)
# Linux (AL2) nodes for general workloads; Windows (Server 2019 Core) nodes
# tainted with os=windows:NoSchedule. Staging uses m6i family for
# production-grade instance uplift from t3 in dev/test.
