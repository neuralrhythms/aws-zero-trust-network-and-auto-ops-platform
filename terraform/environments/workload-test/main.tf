################################################################################
# Environment: workload-test
# Account role: Integration and functional testing (Workloads BU OU)
#
# Module composition:
...
# ------------------------------------------------------------------------------
# EKS cluster — heterogeneous managed node groups
# Linux (AL2) nodes for general workloads; Windows (Server 2019 Core) nodes
# tainted with os=windows:NoSchedule so only Windows-tolerating pods land there.
# Instance types sourced exclusively from var.linux_instance_types and
# var.windows_instance_types — never hardcoded here (REQ-5.5).
# ------------------------------------------------------------------------------
module "eks_cluster" {
...
# Fluent Bit is configured via fluentbit-values.yaml in the module to ship
# container logs to Splunk Cloud HEC. The HEC token is sensitive and must
# be sourced from AWS Secrets Manager at deployment time — never hardcoded.
# ------------------------------------------------------------------------------
module "eks_addons" {
